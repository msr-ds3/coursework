...
Repeat this procedure adding a quadratic term (k=2) for the square of the minimum temperature, and then a cubic term (k=3), etc., up to whatever degree polynomial you feel is appropriate. Hint: the ``poly()`` function is probably useful here. When you’ve fit and evaluated each of these models, make a plot of the correlation between predicted and actual values as a function of the degree of the polynomial, with one curve for the training data and one for the test data. Use this plot to determine the polynomial degree that gives the best predictive performance on the test set.
