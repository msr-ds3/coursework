...
year_counts <- read.csv("year_counts.tsv", header = FALSE, col.names = c("term","year","volume","book_count"), sep = "\t")

total_counts <- read.csv("total_counts.csv", header = FALSE, sep = ",", col.names = c('year', 'total_volume', 'page_count', 'book_count'))
...
total number of row of "total_counts" is `r nrow(total_counts)`
total number of row in "year_counts" is `r nrow(year_counts)`
...
merged_counts <- merge(year_counts, total_counts, by = 'year')
merged_counts <- merged_counts %>% mutate( proportion = volume / total_volume)
...
merged_counts %>% 
  ...
  ggplot(aes(x = year, y = sum_prop, color = factor(term))) + 
  geom_line(size = 1.2)
...
Answer: Yes, our figure illustrates quite similar figure as Michel et al. 2011
...
Add your screenshot for Part C below this line using the `![](screenshot.png)` syntax and comment on similarities / differences.

'![Ngram Viewer Screenshot](screenshot.png)'
...
merged_counts %>% 
  filter( year > 1850 & year <= 2012) %>%  
  filter(term == 1883 | term == 1910 | term ==1950) %>% 
  group_by(year, term, volume) %>%
  summarise(volume_by_year = sum(volume))%>% 
  ggplot(aes(x = year, y = volume_by_year, color = factor(term))) + 
  geom_line(size = 1.2)
...
merged_counts %>% 
  filter( year > 1850 & year <= 2012 & (term == 1883 | term == 1910 | term ==1950)) %>% 
...
in Part D, total number of years mentioned in paper seemed like it is keeping its volume over the years compared to Part B where it seemed like it was decreasing. In part B, we compared to total volume of all papers. It lead me to reevaluate the results. It is true that more papers are published as year increases but does it really mean we forget the fast or are there more to remember?
...
```{r compute-peaks}
peak_year <- merged_counts %>% 
  group_by(term) %>% 
  mutate(max = max(proportion), max_year = year ) %>%
  filter(max == proportion)
```
...
half_life <- left_join(
  merged_counts, peak_year[c("max_year","term", "max")], by =c("term")) %>% 
  group_by(term) %>%
  mutate( year_diff = year - max_year ) %>% 
  filter( year_diff > 0 & proportion <= max / 2) %>% filter(year_diff == min(year_diff))
```
...
Answer: In different time frame, the figure we came with does not match with Mitchel et al. d 
...
## Your written answer
There are many outliers between 1980 and 195. Around beginning of 1900, most outliers are observed, especially with bigger half-lives.

Write up your answer to Part G here. Include code that shows the years with the smallest and largest half-lifes. The possible explaination would be the year where the war started might cause higher half-lives.
max half_life:
`r max(half_life$year_diff)` 
min half_life:
`r min(half_life$year_diff)`
