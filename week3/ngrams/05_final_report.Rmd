...
author: "Nabin Oli (Caldwell University)"
date: '`r Sys.time()`'
output:
  html_document:
    toc: yes
...
There are total of `r nrow(year_table)` in year.csv and there are `r nrow(total_counts_table)` rows in total.tsv
# Part B
...
Yes the graph looks similar to that on figure 3a of Michel et al.(2011)
...
We might conclude that people are actually remembering more as year passes on by looking at part D as there is increase in the occurance of terms with increasing year. But paper looks occurance as a ratio to total counts of all ngrams that year and from plot-total, there is increase in number of total counts per year so we don't need to reevaluate any of the results of the paper.
...
It doesn’t have a same shape as in paper. There is no clear trend with increase in years. There are few outlier years that push and pull the line of fit.
# Part G
...
- 1900 is the top one because it is a start of new centuary.
