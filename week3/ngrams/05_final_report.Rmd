...
names(year_counts) <- c("term", "year", "volume", "book_count")

total_counts <- read_csv("total_counts.csv")
names(total_counts) <- c("year", "total_volume", "page_count", "book_count")
...
year_total <- year_counts %>% inner_join(total_counts, join_by(year))

proportion_table <- year_total %>%
  group_by(term, year) %>%
  summarize(appearances_per_year = sum(volume), proportion = appearances_per_year/total_volume) 
...
proportion_table %>%
  filter(year > 1800 & term == c(1883, 1910, 1950)) %>% 
  ggplot(aes(x=year, y=proportion)) +
  geom_line(aes(color=as.factor(term))) + 
  labs(x="Year", y="Frequency") +
  scale_y_continuous(label=label_percent(scale=10000, suffix="")) + 
  guides(color=guide_legend(title=""))

```
#Unfiltered Version (for all years)
```{r}
proportion_table %>%
...
We found that there were years not included in the published version, which included spikes in years cut off from our table. Other than that, the general trenfs from around 1833 to 2000 seemed to stay the same between both versions. 
...
We found there was similarities in the patterns from the years of around 1800 to the present, but the peaks for years before 1800 were different between our graph and the screenshot.
...
raw_count_table <- year_total %>%
  group_by(term, year) %>%
  summarize(appearances_per_year = sum(volume))

raw_count_table %>%
  filter(term == c(1883, 1910, 1950)) %>% 
  ggplot(aes(x=year, y=appearances_per_year)) +
  geom_line(aes(color=as.factor(term))) + 
  labs(x="Year", y="Number of Appearances") +
...
Using the above plot, we can see how rapidly the total number of words published per a year has been increasing. Using this information, we may be able to explain the differences between our two figures (the normalized vs the raw counts) by showing that it may be less that we are "forgetting faster" and moreso that since we are publishing so much more, proportions are decreasing because the number of words is also increasing so rapidly. 
...
half_life <- 
  proportion_table %>%
  filter(year > 1850 & year < 2012) %>%
  inner_join(peaks, join_by(term)) %>%
  group_by(term) %>%
  filter(proportion <= peak_proportion/2 & year>peak_year) %>%
  summarize(half_life = min(year)-peak_year)
...
The results not that similar. The dips and peaks of the graph don't really match the depth of the one in the paper, but it does have the same general downwards trend as the years get closer to 200.
...
## Your written answer
As we can see from the code snippets, below, we can assume that the years with smaller half lives were less eventful than those with the largest. We also can see that the largest halflives come with years before 1900, bringing us back to the point of proportions vs raw total counts, making us interested to see if we would get similar results with a different measure than proportions .
