...
proportion <- inner_join(total_counts, year_counts, by='year') %>% mutate('prop' = volume/total_volume, 'term_factor' = factor(term))
...
prop_filter <- proportion %>% 
  filter(term == '1883' | term == '1910' | term == '1950') %>%
    filter(year >= 1850 & year <= 2012)


prop_filter %>% 
  ggplot(aes(x = year, y = prop, color = term_factor)) +
  geom_line() +
...
![](/Users\hadassah\Documents\GitHub\coursework\week3\ngrams\ngrams.png)
...
prop_filter %>% 
  ggplot(aes(x = year, y = volume, color = term_factor)) +
  geom_line() +
...
i don't feel the need to reevaluate previous results because while the mentions are increasing over time, the percentage graph shows the proportion of usage (which is much more representative of the whole).
...
peaks <- year_terms %>% 
  rename("peak_prop" = "prop", "peak_year" = "year") %>%
  group_by(term) %>% 
  arrange(desc(peak_prop)) %>%
  top_n(1) %>%
  select(term, peak_year, peak_prop)
...
half_prop <- peaks %>%
  mutate(half_peak = peak_prop/2)

half_lifes <- left_join(year_terms, half_prop, by = "term") %>% 
  select(term, year, peak_year, half_peak, prop) %>% 
  group_by(term) %>% 
...
```{r plot-half-lifes}
half_lifes %>%
  ggplot(aes(x = year, y = half_life)) +
  geom_point() +
  geom_smooth()

# half_life_all_years <- proportion %>% 
#   filter(year >= 1850 & year <= 2012) %>% 
#   filter(term >= 1875 & term <= 1975) %>% 
#   group_by(term) %>% 
#   mutate(peak_proportion = max(prop)) %>% 
#   filter(proportion_of_total <= peak_prop/2 & year > term) %>%
