...
```{r load-counts}
df_yc=read_tsv("year_counts.tsv",col_names=c("term","year","volume","book_count"))
df_total=read_csv("total_counts.csv",col_names=c("year","total_volume","page_count","book_count"))

df_yc["term"]=as.factor(df_yc$term)
...
#df_yc%>%group_by(year)%>%summarize(count=n())
df=inner_join(df_total,df_yc,by="year")
df %>% mutate(prop=volume/total_volume)
```
...
df %>% mutate(prop=volume/total_volume) %>%
  filter(year>=1850 &year<=2012 & term %in% c(1883,1910,1950))%>%
  ggplot(aes(x=year,y=prop,color=term))+
    geom_line()+
...
version 2 of the NGram data produces similar results.
...
```{r compute-peaks}
df_maxP=df %>% mutate(prop=volume/total_volume) %>%
  filter(year>=1850 &year<=2012 & term %in% seq(1875,1975))%>%
  select(term,max_yr=year,max_p=prop)%>%
  group_by(term)%>%
  arrange(term,desc(max_p))%>%
  top_n(1,wt=max_p)
```
...
half_values=df_maxP %>%  mutate(half_p=1/2*max_p) 
half_yrs=df %>% mutate(prop=volume/total_volume) %>%
  inner_join(half_values,by="term")%>%
  select(term,year,max_yr,max_p,half_p,prop)%>%
  filter(prop<=half_p,year>max_yr)%>%
  group_by(term)%>%
  arrange(term,year)%>%
  top_n(-1,wt=year)%>%
  select(term,max_yr,half_yr=year)%>%
  mutate(yrs=half_yr-max_yr)
...
```{r plot-half-lifes}
half_yrs%>%
  filter(term %in% seq(1850,2012))%>%
  ggplot(aes(x=as.numeric(as.character(max_yr)),y=yrs))+
  geom_point()+
  ylab("Half Years")+
  xlab("Years")+
  geom_smooth()
