...
```{r number-of-lines}
count(year_counts) # 53393
count(total_counts) # 425
...
join_year_totals <- inner_join(year_counts, total_counts, "year") %>%
  mutate(frequency = volume / total_volume)
...
frequency_year <- join_year_totals %>%
...
Does version 2 of the NGram data produce similar results to those presented in Michel et al. (2011), which are based on version 1 data?
  They are very similar looking. The data is a little lower
  In our data (version 2), '1950' goes down a little and then peaks again in around 1954. In their data (Version 1), the red line only goes up until it falls. 
  The green line has more peaks on the fall going down in the paper then ours does. 
  The blue in our data seems to go higher than theres. 
...
There is way more smooth. Both have the same general graphs. 
...
Write up your answer to Part E here.
    The difference does lead us to reevalute. The number of things printed every year is increase at an exponential rate. This is shown in the plot above 
  It might seem like things are being forgotten but there is way more printed - a larger denominator. This shows a faster decrease in the string '1950'. The reason it can be going down quicker and seemingly like people are forgetting is beacsue there is more material printed. 
...
half_max <- right_join(frequency_year, max_peaks, "term") %>%  # term, year, year frequency, max_year, max_year_frequency. 
  
  filter(frequency.x <= frequency.y / 2 & year.x > year.y) %>% # before the max year
  
  group_by(term) %>%
  
  filter(year.x == min(year.x)) %>%
  mutate(half_life = year.x - year.y)
...
```{r plot-half-lifes}
# I know something is wrong here to calculate the half life for each year 

half_times <- right_join(frequency_year, max_peaks, "term") %>%
  filter(frequency.x <= frequency.y / 2 & year.x > year.y) %>%
  group_by(term) %>%
  
  mutate(half_life = year.x - year.y) %>%
  ungroup() %>%
  mutate(everyline = (year.x - min(year.x)) / half_life)
  
# plot of minimum number of years it takes for the proportion of mentions to decline from its peak value to half its peak value
ggplot() +
  geom_point(data = half_max, aes(x = year.x, y = half_life, colour = factor(term))) + 
  scale_colour_manual(values = c("blue", "green", "red"))
# plot of each years half life
ggplot() + 
  geom_point(data = half_times, mapping = aes(x = year.x, y = half_life)) + 
  geom_smooth(data = half_times, mapping = aes(x = year.x, y = half_life))
