...
The total number of lines in `total_counts` is `r count(total_counts)`. The total number of lines in `year_counts` is `r count(year_counts)`.
...
ngrams_years_totals <- left_join(year_counts, total_counts, by=c('year'='year'))
ngrams_years_totals <- mutate(ngrams_years_totals, prop_volume = volume/total_volume)
```
...
ngrams_years_totals %>% 
  ...
  ggplot(aes(color = term, x = year, y = prop_volume)) +
  geom_line() +
...
ngrams_years_totals %>% 
  filter(term %in% c("1883", "1910", "1950"), year >= 1850, year <= 2012) %>% 
  ggplot(aes(color = term, x = year, y = volume)) +
  geom_line() +
  scale_y_continuous(labels = scales::percent)
```
...
```{r compute-peaks}
peaks <- ngrams_years_totals %>% 
  filter(year >= 1850, year <= 2012) %>% 
  group_by(term) %>% 
  arrange(term, desc(prop_volume), year) %>% 
  top_n(1) %>% 
  select(term, peak_year = year, peak_frac = prop_volume) %>% 
  mutate(half_peak = peak_frac/2)
...
half_lifes <- left_join(ngrams_years_totals, peaks, by=c('term'='term')) %>% 
  filter(year >= 1850, year <= 2012) %>% 
  group_by(term) %>% 
  filter(year >= peak_year, prop_volume <= half_peak) %>% 
  # arrange(year) %>% 
  # top_n(1) %>%
  summarise(half_year = min(year),
            peak_year = min(peak_year)) %>% 
  mutate(half_life = half_year - peak_year) %>%
...
```{r plot-half-lifes}
half_lifes %>% ggplot(aes(x =  as.numeric(term) , y = half_life)) +
  geom_point() +
  geom_smooth()
