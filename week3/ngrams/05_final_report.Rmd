...
```{r plot-totals}
join %>% group_by(term,year) %>% summarize(term_count = sum(volume) , total_count = sum(total_volume)) %>% f
ilter((term == 1883 | term == 1910 | term == 1950) & (year >= 1850 & year <= 2012))%>% 
...
```{r compute-peaks}
peak <- join %>%  group_by(year,term) %>% summarize(term_count = sum(volume) , total_count = sum(total_volume)) %>% 
...
min <- join %>%  group_by(term,year) %>% summarize(term_count = sum(volume) , total_count = sum(total_volume)) %>%
mutate( proportion = term_count/total_count) 
min
peak_year <- min %>% left_join(peak) %>%  filter(year>= 1875 , year <= 1975) %>% filter(proportion == peak) %>% summarize(peakyear= year ,term,peak)
peak_year 
...
```{r plot-half-lifes}
min %>%  inner_join(peak_year, by = "term")  %>% filter((year > peakyear)& (proportion <= peak/2 )) %>% mutate( half_life = year - peakyear) %>%
group_by(term) %>% summarize(half_life = min(half_life)) %>% filter(term>=1850 & term<=2012) %>%
...
Yes there are years that are fogotten too quickly like and as you can see in the code the outliers here are 1900 being forgotten too slowly and 1942 being forgotten too quickly 
...
where there wenta any significant policies or moves that were pivotal 

``` {r min-max-halflife }
min %>%  inner_join(peak_year, by = "term")  %>% filter((year > peakyear+1)& (proportion <= peak/2 )) %>% mutate( half_life = year - peakyear) %>%
group_by(term) %>% summarize(half_life = min(half_life)) %>% filter(term>=1850, term<=2012) %>% filter(half_life == min(half_life) | half_life == max(half_life)) 
```
