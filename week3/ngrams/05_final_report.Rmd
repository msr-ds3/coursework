...
library(readr)  
library(here) 
year <- read_tsv((here('week3', 'year_counts.tsv')),
                    col_names = c('term', 'year', 'volume',  'book_count'))

counts <- read_csv((here('week3','total_counts.csv')),
...
> Recreate the main part of figure 3a of Michel et al. (2011). To recreate this figure, you will need two files: the one you downloaded in part (a) and the “total counts” file, 
which you can use to convert the raw counts into proportions. Note that the total counts file has a structure that may make it a bit hard to read in.
...
join <- year %>% inner_join(counts, by = "year" )
join
join %>% group_by(year,term) %>% 
summarize(term_count = sum(book_count.x) , total_count = sum(book_count.y))%>% 
mutate( proportion = term_count/total_count)
...
library(scales)
join %>% group_by(year,term) %>% summarize(term_count = sum(volume) , total_count = sum(total_volume))%>% 
mutate( proportion = term_count/total_count) %>% filter((term == 1883 | term == 1910 | term == 1950) & (year >= 1850 & year <= 2012))%>% 
ggplot(aes(x = year, y = proportion, color = factor(term)))+ geom_line() + scale_y_continuous(label = percent)+
xlab('Years') +ylab('Frequency')
```

## Your written answer

There is no differnece between the graphs for version 2 and version 1 for the proportion of mentions for each term normalized by the total counts for each year 
...
There is a difference between the graphs of Version 2 and the NGram viewer for the proportions of mentions for each term as the all three terms each have higher proportions for each year
...
join %>% group_by(term,year) %>% summarize(term_count = sum(volume) , total_count = sum(total_volume)) %>% 
filter((term == 1883 | term == 1910 | term == 1950) & (year >= 1850 & year <= 2012))%>% 
ggplot(aes(x = year, y = term_count, color = factor(term)))+ geom_line() +scale_y_continuous(label = comma)+
xlab('Years') +ylab('Mentions')
...
join %>% group_by(term,year) %>% summarize(term_count = sum(volume) , total_count = sum(total_volume)) %>% 
filter((term == 1883 | term == 1910 | term == 1950) & (year >= 1850 & year <= 2012))%>% 
ggplot(aes(x = year, y = total_count ))+ geom_line() +scale_y_continuous(label = comma)+
xlab('Year') +ylab('Total Counts')
```


Write up your answer to Part E here.

They dont lead me to reevaluate the results of Michel et al. (2011) because these are just summs of the the counts but the what Michel et al. (2011) are showing is the 
proportion bascially the relativity to the total in each group so it may look different. Also i think that the total count is going to keeep that kind of trend upward because 
more books are being released so its just a summation but it does eventually start to decrease so i dont think there is a difference

# Part F

> Now, using the proportion of mentions, replicate the inset of figure 3a. That is, for each year between 1875 and 1975, calculate the half-life of that year. The half-life is defined to be the number of years 
that pass before the proportion of mentions reaches half its peak value. Note that Michel et al.
(2011) do something more complicated to estimate the half-life—see section III.6 of the Supporting Online Information—
but they claim that both approaches produce similar results. Does version 2 of the NGram data produce similar results to those presented in Michel et al. (2011),
which are based on version 1 data? (Hint: Don’t be surprised if it doesn’t.)
...
```{r compute-peaks}
peak <- join %>%  group_by(year,term) %>%  filter(term>= 1875 & term <= 1975)  %>%  filter(year >= 1850 & year <= 2012) %>% summarize(term_count = sum(volume) , total_count = sum(total_volume)) %>%
mutate( proportion = term_count/total_count) %>% group_by( term)%>% summarize (peak = max(proportion))
peak
...
min <- join %>%  group_by(year,term) %>%  filter(year >= 1850 & year <= 2012) %>% summarize(term_count = sum(volume) , total_count = sum(total_volume))  %>%  filter(term>= 1875 & term <= 1975) %>%
mutate( proportion = term_count/total_count) 
min
peak_year <- min %>% left_join(peak, by= "term") %>%  group_by(term) %>% filter(proportion == peak) %>% summarize(peakyear = max(year) ) %>%left_join(peak, by = "term")
min %>%  left_join(peak_year, by = "term")  %>% filter((year > peakyear)& (proportion <= peak/2 )) %>% mutate(half_life = year - peakyear) %>%
group_by(term) %>% summarize(half_life = min(half_life))
```
...
min %>%  left_join(peak_year, by = "term")  %>% filter((year > peakyear)& (proportion <= peak/2 )) %>% mutate(half_life = year - peakyear) %>%
group_by(term) %>% summarize(half_life = min(half_life))%>%
ggplot(aes(x = term, y = half_life))+geom_point()+geom_smooth()+
xlab('Term') +ylab('Half-Life')

```
Write up your answer to Part F here.

 Version 2 does not provide a similar results to those of Michel et al. (2011). Version 2 seems to trend a little more regularly than 
 Michel et al. (2011) whisch looks like its tredning more downward. 
...
Yes there are years that are fogotten too quickly like and as you can see in the code the most extreme outliers here are 1900 being forgotten too slowly and 1942 being forgotten too quickly 
this is because in 1900 it was the beginning of the century so it was very popular and then for 1942, i think it was forgotten too quickly because it was a time during world war 2 
where there wenta any significant policies or moves that were pivotal. Other years that were in the bottom like 1877 and 1878 there was nothing exciting just economic policies that did not benefit the people at that time 

``` {r min-max-halflife }
#Top  
min %>%  inner_join(peak_year, by = "term")  %>% filter((year > peakyear)& (proportion <= peak/2 )) %>% mutate( half_life = year - peakyear) %>%
group_by(term) %>% summarize(half_life = min(half_life)) %>% filter(term>=1850 & term<=2012) %>% arrange(desc(half_life))
#Bottom
min %>%  inner_join(peak_year, by = "term")  %>% filter((year > peakyear)& (proportion <= peak/2 )) %>% mutate( half_life = year - peakyear) %>%
group_by(term) %>% summarize(half_life = min(half_life)) %>% filter(term>=1850 & term<=2012) %>% arrange(half_life)
``` 
# Makefile
