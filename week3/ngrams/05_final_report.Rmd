...
total_counts <- read_csv("C:/Users/Abhishek/Documents/coursework/week3/ngrams/total_counts.csv")
years_count <- read_tsv("C:/Users/Abhishek/Documents/coursework/week3/ngrams/years_count.tsv")
...
df_with_proportion <- year_counts_totals %>%
...
The Version 2 of the Ngram data produces similar results to those presented in Michel et al. (2011) which were based on version 1 data.   
...
The scale is different for the screenshot graph, and the data percentage for each term year are almost double the amount from our graph above. This leads us to assume that the google ngrams has high number of data than the data we have which lead to a smaller graphing figure.
...
Part B led us to conclude that the proportion of usage of those specific term year started to declined after 2000. Looking at Part D, the assumption was inverse. In-general, considering the total volume, the usage of those years have almost doubled and have been increasing lately. As we can see in Part E, how the total usage volume has sky rocketed from 1975 eventually. 
...
half_life <- max_proportion %>%
    mutate(half_of_max = maximum_proportion/2)%>%
    rename('peak_year' = 'year')
View(half_life)

new_half <- left_join(df_with_proportion, half_life, by = 'term')%>%
  filter(proportion <= half_of_max & year > peak_year)%>%
  group_by(term)%>%
  filter(proportion == max(proportion))%>%
  summarize(term, year)
...
```{r plot-half-lifes}
final_half_life <- merge(max_proportion, new_half, by = 'term') %>%
...
Version 2 of the NGram data does not produce similar research results to those presented in Michel et al. (2011), which were based on version 1 data. We assume that Michel et al. (2011) adopted the complicated process to make the graph precise but we followed simpler steps. 
...
From Part F we can notice outliers. The year 1900 and 1975 were forgotten very slowly in comparison to other term years. Inversely, 1940, 1914 and 1915 were quickly forgotten. The possible reason could be the major event that happened in the world. 


half_life %>% 
  arrange(desc(half_of_max))%>%
  head()
