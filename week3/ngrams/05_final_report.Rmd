...
```{r load-counts}
year_data <- read_tsv('year_counts.tsv', col_names = c('term','year','volume','book_count'))
tot_data <- read_csv('total_counts.csv', col_names = c('year', 'total_volume', 'page_count', 'book_count'))
...
ngram_all <- inner_join(year_data, tot_data, by = c('year'='year'))
head(ngram_all)

ngram_proportion <- ngram_all %>% mutate( proportion = volume / total_volume ) 
# %>% select(term, year, volume, total_volume, proportion) 
head (ngram_proportion)
```
...
cleaned_graph <- ngram_proportion %>% 
  filter(term == "1883" | term == "1910"| term == "1950") 

cleaned_graph %>% ggplot((aes(x = year, y= proportion ))) + geom_line(aes(color = term)) + labs(x = 'Year', y = 'Frequency', color = 'Term') + scale_color_manual(values = c("Blue", "Green", "Red")) + coord_cartesian(xlim = c(1850, 2012)) 

#+ percent( ngram_proportion$year, scale = 1000 )
...
Looks more or less the same. Couldn't get the scale function to work.
...
cleaned_graph %>% 
ggplot(aes(x=year, y = volume)) + geom_line(aes(color = term)) + scale_y_continuous(label = comma) + labs(x = "Year", y = "Raw Mentions", color = "Term") + scale_color_manual(values = c("Blue", "Green", "Red")) + coord_cartesian(xlim = c(1850, 2012)) 
...
cleaned_graph %>% 
ggplot(aes(x=year, y = total_volume)) + geom_line() + scale_y_continuous(label = comma) + labs(x = "Year", y = "Total Count") + coord_cartesian(xlim = c(1850, 2012)) 
...
It seems he's misguided. The first graph (b) plots the proportion while the second the Raw count. We can see from the last graph that the total count has gone up exponentially in recent years most probably due to the ability to easily publish articles online and therefore it is to be expected that the proportion would drop while the raw count stays steady. I wouldn't jump to say it's because people are forgetting faster. 
...
terms <- ngram_proportion %>%
filter(year > 1849 & year < 2013) %>% filter(term %in% c("1850":"2012"))
#head (terms)

peak_terms <- terms %>% group_by(term) %>% summarize(proportion = max(proportion))
#head(peak_terms)

peak_terms <- left_join(peak_terms, terms, by = "proportion")
#head(peak_terms)

peak_terms <- peak_terms %>% transmute(term = term.x, peak_year = year.x, peak = proportion)
head(peak_terms)
...
half_life <- peak_terms %>% mutate(half_life = peak / 2 )

half_life <-  left_join(terms, half_life, by = "term") %>% filter(proportion <= half_life & year > peak_year) %>% group_by(term) %>% summarize(proportion = max(proportion))

half_life <- left_join(half_life, terms, by = "proportion")

half_life <- half_life %>% transmute(term = term.x, half_peak_year = year, half_peak = proportion)

joined_tip <- left_join(peak_terms, half_life, by = "term")

joined_tip <- joined_tip %>% mutate(min_years = half_peak_year - peak_year, year_term = as.numeric(term))
...
```{r plot-half-lifes}
joined_tip %>%
  ggplot(aes(x = year_term, y = min_years)) + geom_point() + geom_smooth() +
  scale_x_continuous(limits = c(1850, 2012)) +  labs(x = "Term", y = "Half-life")
```
...
Yes. In the 1850's and 1900's. Maybe because of big wars? 
