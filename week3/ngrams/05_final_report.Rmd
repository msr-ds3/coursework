...
```{r load-counts}
year_counts <- read.csv("year_counts.tsv", header = FALSE, sep = "\t", col.names = c('term', 'year', 'volume', 'book_count'))
...
**The 'total_counts' data frame has `r nrow(total_counts)` rows**

**The 'year_counts' data frame has `r nrow(year_counts)` rows.**
...
joined_data <- full_join(total_counts, year_counts, by= "year")
head(joined_data)
...
filtered_data_1850_2012 <- joined_data |>
  filter(year>= 1850 & year<= 2012)
```
```{r}
filtered_data_1850_2012 |> 
  filter(term ==1883 | term == 1910| term == 1950) |>
  group_by(year, term, proportion) |>
...
Our replicated graph and graph from the book looks almost same only noticeable difference is our graph goes beyond year 2000 where book graph only went until year 2000 
...
![](figure_ngram_viewer.png)

syntax and comment on similarities / differences.

Both mine & the ngram viewer graph has similar pattern but the website graph is very much smoother where mine has alots of spikes
...
filtered_data_1850_2012 |> 
  ...
  summarise(sum_mention= sum(volume), na.rm = TRUE)  |> 
  ggplot(aes(x = year, y = sum_mention, color = factor(term))) +
  geom_line() +
  scale_y_continuous(labels = comma) +
  labs(title = "Mentions Over Time",
       x = "Year",
       y = "Sum of Mentions")
...
filtered_data_1850_2012 |> 
  filter(term ==1883 | term == 1910| term == 1950) |> 
  group_by(year, term, volume) |> 
  summarise(sum_total= sum(total_volume), na.rm = TRUE)  |> 
...
```{r compute-peaks}
peaks_data <- filtered_data_1850_2012 |> 
  # filter(term ==1883 | term == 1910| term == 1950) |>
  group_by(term) |>
  slice_max(proportion) 

  colnames(peaks_data)[which(names(peaks_data) == "proportion")] <- "max_proportion"
  colnames(peaks_data)[which(names(peaks_data) == "year")] <- "max_year"
...
half_life <- left_join (filtered_data_1850_2012, peaks_data, by = "term") |>
  filter(proportion <= max_proportion/2 & year > max_year) |>
  group_by(term) |>
  slice_min(year) |>
  mutate(min_half_life = year - max_year) |>
  select(year, term, max_year, min_half_life )
  
  colnames(half_life)[which(names(half_life) == "year")] <- "half_life_year"
...
```{r plot-half-lifes}
half_life |>
  ggplot(aes(x = term, y= min_half_life)) +
  geom_point() +
...
**smallest half life :** `r min(half_life$min_half_life)`

**largest half life :** `r max(half_life$min_half_life)`
