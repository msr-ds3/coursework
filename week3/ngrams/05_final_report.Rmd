...
```{r load-counts}
total_count <- read.csv("total_counts.csv",header = FALSE, sep = ",", dec = ".")
year_counts <- read.delim("year_counts.tsv",header = FALSE, sep = "", dec = ".")
colnames(year_counts)[3] <- "volume"


#keeps <- c("V1","V2")
total_counts = total_count[keeps]

colnames(total_counts)[2] <- "total_volume"
colnames(total_counts)[1] <- "year"
...
years_and_total_count <- left_join(year_counts, total_counts, by="year")
```
...
proportions<-years_and_total_count%>%
  filter(year >= term )%>%
  group_by(year)%>%
...
proportions%>%
  ...
  mutate(proportion = proportion*10000)%>%
  ggplot(aes(x = year, y = proportion))+ geom_line(aes(color = as.character(term)))+
...
```{r plot-half-lifes}
max_prop<-proportions%>%
  group_by(term)%>%
  filter(proportion == max(proportion))

colnames(max_prop)[6] <- "peak_prop"
colnames(max_prop)[2] <- "peak_year"

keeps <- c("term","peak_year","peak_prop")
peak_prop = max_prop[keeps]

all_proportions <- left_join(proportions, peak_prop, by="term")

all_proportions%>%
  filter(year >= peak_year, proportion <= peak_prop/2)%>%
  group_by(term, peak_year)%>%
