...
# load in year_counts.tsv and store data in dataframe named year_counts
...
# load in total_counts.csv and store data in dataframe named total_counts

total_counts <- read_csv(here('total_counts.csv'), 
...
The year_counts dataframe has `nrow(year_counts)` and the total_counts dataframe has `nrow(total_counts)`.
...
term_counts_with_total_counts <- year_counts %>% 
  left_join(total_counts, by="year") %>%
  mutate(proportion_of_mentions = volume / total_volume) %>%
  mutate(term = factor(term)) # force the term to be a factor

View(term_counts_with_total_counts)
...
Yes, version 2 of the NGram data produces similar results to those presented in Miches et al. (2011), which are 
based on version 1 data. Based on both version 1 data and version 2 data, the graph for each year peaks at that
year, then levels off in the following years. 
...
The graph produced by the ngram viewer is consistent with the graphs from the version 1 data and the version 2 data.
...
# inline code with a link to the screenshot
`![screenshot] (years_screenshot.PNG)`
...
term_counts_with_total_counts %>%
  ...
  scale_color_manual(values=c('blue', 'green', 'red')) +
  scale_x_continuous("Year", limits = c(1800, 2000)) +
  scale_y_continuous("Raw Counts", label=comma)
...
total_counts %>% 
  ...
  scale_y_continuous("Total  Count", label=comma)
...
Yes, the difference between (b) and (d) leads me to reevaluate the results of Michel et al.
(b) shows an increase in frequency for each word, then a gradual decrease. 
(d) shows an increase in the raw count for each word, then a decrease then an increase. 
As demonstrated in (e), the total count has been increasing sharply, 
so even though the raw count for specific words has been increasing, as shown in (d),
the proportion of the total words that each individual word comprises, is decreasing.
Michel et al. claim that the frequency is decreasing, but their results do not 
take into account the fact that this trend may be attributed, at least in part, to
the fact that the total count is increasing so dramatically. 
...
# dataframe of peak count for each term

peak_prop_per_term <- term_counts_with_total_counts %>%
  select(term, year, volume, total_volume, proportion_of_mentions) %>%
  filter(year >= 1800) %>%
  group_by(term) %>% 
  filter(proportion_of_mentions == max(proportion_of_mentions)) %>%
  select(term, year, proportion_of_mentions) %>% 
  rename(peak_prop = proportion_of_mentions, peak_year = year)

View(peak_prop_per_term)
  
# combine term_counts_with_total_counts dataframe with peak_prop_per_term dataframe

peak_with_term_and_year <- term_counts_with_total_counts %>%
  filter(year >=1800) %>%
  left_join(peak_prop_per_term, by="term") %>% 
  select(term, year, proportion_of_mentions, peak_year, peak_prop)

View(peak_with_term_and_year)
...
half_life_df <- peak_with_term_and_year %>%
  group_by(term) %>%
  filter(year > peak_year) %>%
  filter(proportion_of_mentions <= peak_prop / 2) %>%
  mutate(half_life = min(year) - peak_year)
...
half_life_df %>%
  filter(year >= 1850 & year <= 2012 & year > as.numeric(term)) %>%
  ggplot(aes(x=as.numeric(as.character(term)), y=half_life)) +
  geom_point() + 
  scale_x_continuous("Year") +
  scale_y_continuous("Half-life (yrs)") +
  geom_smooth()
...
Version 2 of the NGram data does not produce similar results to those presented in 
...
The years 1803 and 1810 had the shortest half-lifes of 1 year.
The year 1800 had the longest half-life of 23 years.

Reasons?

```{r code for years with smallest and largest half-lifes}
half_life_df %>%
  ungroup() %>%
  filter(half_life == min(half_life) | half_life == max(half_life)) %>%
  View
```
