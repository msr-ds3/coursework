...
```{r load-counts}
year_counts <- read_tsv(here('week3/ngrams/year_counts.tsv'), col_names = c('ngram', 'year', 'count', 'distinct_count'))
total_counts <- read_csv(here('week3/ngrams/total_counts.csv'), col_names=c('year', 'match_count', 'page_count', 'volume_count'))
...
year_counts_proportion <- year_counts |>
    inner_join(total_counts, by="year") |>
    mutate(proportion=count / match_count)
...
year_counts_proportion |>
    filter(ngram %in% c(1883, 1910, 1950), year %in% 1850:2012) |>
    mutate(ngram = as.factor(ngram)) |>
    ggplot(aes(x=year, y=proportion, color=ngram)) +
    geom_line() +
...
![](google_books_screenshot.png)

This version has a much steeper rate of descent than we had.
...
year_counts |>
    filter(ngram %in% c(1883, 1910, 1950), year %in% 1850:2012) |>
    mutate(ngram = as.factor(ngram)) |>
    ggplot(aes(x=year, y=count, color=ngram)) +
    geom_line() +
    scale_color_manual(values=c("blue", "green", "red")) +
...
```{r plot-totals}
total_counts |>
    filter(year %in% 1850:2012) |>
    ggplot(aes(x=year, y=match_count)) +
    geom_line() +
    scale_y_continuous(label=comma)
```
...
The reason why the total count for the old years is going up is because there are so many books (after the invention of the typewriter) that even though those years are being mentioned more, the increase of other years is still greater.
...
```{r compute-peaks}
# We did it in one go, we didn't need this
```
...
half_lives <- year_counts_proportion |>
    filter(year > 1850) |>
    filter(ngram %in% 1875:1975) |>
    arrange(desc(proportion)) |>
    group_by(ngram) |>
    filter(year >= first(year)) |> 
    filter(proportion == max(proportion) | proportion <= max(proportion) / 2) |>
    summarize(half_life = nth(year, 2) - first(year), peak_year = first(year), half_year = nth(year, 2))
```
...
We are getting a slight downward trend, but it's within the margin of error. It seems that they just didn't have a big enough dataset.
...
half_lives |>
    arrange(half_life) |>
    filter(!half_life %in% quantile(half_life, 0.05):quantile(half_life, 0.95))
```

I checked for the years in the upper and lower 5% of half-lives. The year with the largest half-life is 1900, probably because it is a good year to benchmark against. The smallest is 1942, probably because of how significant its neighbors were. Except, 1917 and 1918 also had small half-lives, and they had a lot of important things. So, I'm not really sure what 
...
Questions:
Is it a problem that we did the half-life in one go?
What do you mean that you want halflife from 1850 to 2012? (and the data doesn't extend that far)
For smooth, do you want line or loess?
