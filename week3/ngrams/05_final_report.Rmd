...
peak_matches_df <- group_by(year_total_joined, term) %>%
  summarize(peak_matches = max(volume), half_peak = trunc(peak_matches / 2)) %>%
  left_join(year_total_joined, by = "term") %>%
  select(term, peak_matches, half_peak, year, volume) %>% 
  group_by(term) %>% 
  filter(peak_matches == volume)
```
