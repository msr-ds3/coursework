...
author: "Addina Rahaman (City College of New York)"
date: '`r Sys.time()`'
output:
  html_document:
    toc: yes
...
`r paste("\nyear_counts rows: ", nrow(year_counts))`
`r paste("\ntotal_counts rows", nrow(total_counts))`
...
term_counts <- left_join(year_counts, total_counts, by="year") %>% 
  rename(term_book_count=book_count.x, total_book_count=book_count.y) %>% 
...
The data in Version 2 has similar general shapes and trends for the ngrams 1883, 1910, and 1950, however the frequencies are scaled down by a factor of 1e^-4.
If the scale of the y axis were the same, the peaks for each ngram would generally have a similar year and frequency.
...
It does, because (d) clearly shows an increase of the ngrams 1883, 1910, and 1950 mentioned over the years. In fact, in the year 2012, 1883 and 1910 both have reached past their first peak. 
The metric used in (b) are proportions relative to the year on the x-axis; the graph may make it appear as if the proportion of mentions for each year are all relative to a constant total volume across all years;
however, this would not be the case since there are more books are being published as the years progress, therefore there would be more total word counts every year.
...
```{r compute-peaks}
term_counts_1875_1975 <- term_counts %>% 
  filter(term >= 1875 & term <= 1975) %>% 
  filter(year >= 1850 & year <= 2012)

proportion_peaks <- term_counts_1875_1975 %>% 
  group_by(term) %>% 
  filter(proportion == max(proportion)) %>% 
  select(term, year, proportion) %>% 
  arrange(term) %>% 
  rename(peak=proportion, peak_year=year)
...
half_lives <- proportion_peaks %>% 
  mutate(half_peak=peak/2) %>% 
  left_join(term_counts_1875_1975, by='term') %>% 
  group_by(term) %>%
  filter(year >= peak_year) %>% # years after peak year
  filter(proportion <= half_peak) %>% # on or below half the peak
  slice_min(order_by=year, n=1) %>%  # first year
  summarize(half_life=year-peak_year)

half_lives_1883_1910_1950 <- half_lives %>% 
...
The graph looks very different this time. Version 1 had a downward trend, but Version 2 increases until the middle of 1900-1910 then stabilizes after (with another local minimum close to 1930).
...
There are a cluster of years between 1900-1925 which have long half-lives (perhaps because of WWI).

```{r longest-short-half-lives}
longest_half_lives <- half_lives %>% 
  slice_max(order_by=half_life, n=10) %>% 
  arrange(term)
longest_half_lives 

shortest_half_lives <- half_lives %>% 
  slice_min(order_by=half_life, n=10) %>% 
  arrange(term)
shortest_half_lives

```

After looking at the particular years, it does seem like the late 1800s and early 1900s (mainly until 1910s) have the longest half life. Meanwhile the lowest half lives are concentrated in the 1870s-1880s. 
