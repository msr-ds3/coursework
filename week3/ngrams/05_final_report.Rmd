...
year_counts <- read_tsv(here('week3/ngrams/year_counts.tsv'),
                    col_names = c('ngram','year','count','distinct_count'))

total_counts <- read_csv(here('week3/ngrams/total_counts.csv'),
                    col_names = c('year','match_count','page_count','volume_count'))
...
year_counts_proportion <- year_counts %>%
  ...
  mutate(proportions = count / match_count)
...
years_to_plot <- c(1883, 1910, 1950)

filtered_data <- year_counts_proportion %>%
  filter(ngram %in% years_to_plot ,  (year %in% 1850:2012)) %>%
  mutate(ngram = as_factor(ngram))

filtered_data %>%
  ggplot(aes(x=year, y=proportions, color=ngram)) +
  geom_line() +
...
Even with different data we ended up getting the same results.
...
This version goes down way faster, it's more steeper.
...
The more we advance the easier it is to write books so it is easier to find some words so there is an increase.
...
# we did it right after in one go
...
half_lives <- year_counts_proportion %>%
  ...
  filter(proportions == max(proportions) | proportions <= max(proportions)/2) %>%
  summarize(half_life = nth(year, 2) - first(year))
...
we are getting a downward trend but with a margin of error
...
quantiles <- quantile(half_lives$half_life, probs = c(0.05, 0.95))  
...
year with largest is 1900 with 21 half lifes while the year with less is 1942 with 4 half lifes
# Makefile
...
half life in one go?
1815-2020 while we worked on different data?
do you want line or loess?