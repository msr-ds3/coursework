...
year_counts <- read_tsv(("C:/Users/ds3/Desktop/New folder/coursework/week3/ngrams/year_counts.tsv"), col_names = c("term", "year", "volume","book_count"))
total_counts <- read_csv(here("C:/Users/ds3/Desktop/New folder/coursework/week3/ngrams/total_counts.csv"), col_names = c("year", "total_volume", "page_count", "book_count"))
head(year_counts)
```
...
The total number of line in year_counts.tsv is 'r nrow(year_counts)'
The total number of line in total_counts.tsv is 'r nrow(total_counts)'
...
head(df)
df <- mutate(df, prop = volume/total_volume)
head(df)
```
...
library(ggplot2)
df2 <- df %>% filter(term == "1883" | term == "1910" | term == "1950") %>% filter(year >= "1850" & year <= "2012")
head(df2)

library(scales)

ggplot(df2, aes(x = year, y = prop * 100, color = term)) + geom_line() + xlab('Years') + ylab('Frequency') + scale_y_continuous(labels = scales::percent)

```

## Your written answer

The results are similar but not the same as 1950 reaches 1.5 frequency for Version 2 but in Version 1 graph it doesn't reach 1.5.
Also, our peak for 1883 is more than 0.5 compared to Version 1 graph.
...
`![](file.png)`
The scales are different and they have more data.
...
library(scales)
ggplot(df2, aes(x = year, y = volume, color = term)) + geom_line() + xlab('Years') + ylab('Volume') + scale_y_continuous(label = comma)
```
...
```{r plot-totals}
df3 <- total_counts %>% filter(year >= "1850" & year<="2012")
ggplot(df3, aes(x = year, y = total_volume)) + geom_line() + xlab('Years') + ylab('Volume') + scale_y_continuous(label = comma)
```

## Your written answer

Write up your answer to Part E here.
The proportion decrease as the years increase because there are more things published in coming years because of population increase and publishing being a popular means.
Hence, people are not actually forgetting things because volumnes mentioning that terms still increase after a dip.
...
```{r compute-peaks}
View(df)
df4 <- df2 %>% group_by(term) %>% filter(prop == max(prop)) %>% select(term, year, prop) 
head(df4)
```
...
df5 <- df2 %>% group_by(term) %>% filter(prop == (max(prop))) %>% select(- c(book_count.x, page_count, book_count.y))
df3 <- df2 %>% group_by(term) %>% mutate(prop_min = ((max(prop)/2))) %>% select(-c(volume, total_volume, book_count.x))
head(df3)
df3 <- df3 %>% mutate(diff = abs(prop - prop_min)) %>% mutate(prop_max = max(prop))
df3 <- df3 %>% filter(diff == (min(diff))) %>% select(- c(page_count, book_count.y))
join <- inner_join(df5, df3, by = "term") %>% select(- c(prop.x))
join <- join %>% mutate(year_diff = year.y - year.x)
join$year_diff
```
...
ggplot(join, aes(x= year.y, y = year_diff, color = term)) + geom_point() +geom_smooth()
```
