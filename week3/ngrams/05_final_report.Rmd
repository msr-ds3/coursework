...
```{r load-counts}
yearcounts <- read_tsv('year_counts.tsv')
totalcounts <- read_csv('total_counts.csv')

#name the columns in tsv and csv file 
names(yearcounts) <- c("term", "year", "volume", "book_count")
names(totalcounts) <- c("year", "total_volume", "page_count", "book_count")
...
```{r}
#print total number of lines in each dataframe
nrow(yearcounts)
nrow(totalcounts)
```
...
df <- left_join(yearcounts, totalcounts, by='year')
...
ggplot() + 
  geom_line(data = counts_1883, aes(x = year, y = count_proportion, color = "1883")) +
  geom_line(data = counts_1910, aes(x = year, y = count_proportion, color = "1910")) +
  geom_line(data = counts_1950, aes(x = year, y = count_proportion, color = "1950")) +
  scale_color_manual(values = c("1883" = "navy", "1910" = "green", "1950" = "red"),
                     labels = c("1883", "1910", "1950")) +
  scale_y_continuous(labels = percent_format(), name = "Proportion of mentions")
  labs(x = "Year", y = "Proportion of mentions", color = "Term") +
  theme_minimal()
```
...
![](C:/Users/jane/Documents/GitHub/coursework/week3/ngrams/onegram.png)



```
These 2 graphs are different in the sense that the Google NGram viewer has higher proportion of mentions than our generated graph. The term "1883", "1915","1950" was mentioned 0.010%, 0.008%,  times more
```
...
ggplot() + 
  geom_line(data = counts_1883, aes(x = year, y = volume, color = "1883")) +
  geom_line(data = counts_1910, aes(x = year, y = volume, color = "1910")) +
  geom_line(data = counts_1950, aes(x = year, y = volume, color = "1950")) +
  scale_color_manual(values = c("1883" = "navy", "1910" = "green", "1950" = "red"),
                     labels = c("1883", "1910", "1950")) +
  labs(x = "Year", y = "Number of times mentioned", color = "Term") +
  theme_minimal()
```
...
total_counts <- df %>%
  filter(year >= 1850 & year <= 2012) %>%
  group_by(year) %>%
  summarise(total_count = sum(volume))

# Plotting total counts over time
ggplot(total_counts, aes(x = year, y = total_count)) +
  geom_line(color = "blue") +  
  scale_y_continuous(labels = comma, name = "Total Counts") +
...
The difference between (b) and (d) does lead me to reevaluate the results of Michel et al. (2011). The trends in graph (b) peaks only in the year of year term, then dips quickly after. However, graph (d) shows that those year term peaked in their year, decreased for an amount of time, but got even more mentions later on compared to their first peak. 
...
half_life_term <- df |> 
  ...
  mutate(half_life = min(year) - max_year)
...
```{r plot-half-lifes}
max_proportion_figure3a <- df |> 
  filter(term %in% (1850:2012)) |> 
  filter(year >= 1850 & year <= 2012) |> 
  group_by(term) |> 
  filter(count_proportion == max(count_proportion)) |> 
  rename(max_proportion = count_proportion, max_year = year)
...
figure3a <- df |> 
  filter(term %in% (1850:2012)) |> 
  filter(year >= 1850 & year <= 2012) |> 
  inner_join(max_proportion_figure3a, by="term") |> 
  group_by(term) |> 
  filter(count_proportion <= max_proportion/2 & year > max_year) |> 
  mutate(half_life = min(year) - max_year)

figure3a$term <- as.numeric(figure3a$term)
...
## Your written answer
Version 2 of the NGram data doesn't produce similar results to those presented in Michel et al. (2011), which are based on version 1 data. THe most obvious observation is that half-life data in version 1 ranges from 0 to slightly above 30 while the version 2 data only has max half-life value around 20. 
...
```{r}
smallest_half_life = min(half_life_term$half_life)
largest_half_life = max(half_life_term$half_life)

years_min_half_life <- half_life_term%>% 
  filter(half_life == smallest_half_life)%>%
  slice(1)


years_max_half_life <- half_life_term%>% 
  filter(half_life == largest_half_life)%>%
  slice(1)
...
Year 1887, 1878, 1942 were the outliers with shortest half life and Year 1900 was the outlier with the longest half life. I suppose that there were many major historical events happening in 1990 while there were fewer in 1887, 1878, 1942. 
