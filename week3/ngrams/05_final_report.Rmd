...
```{r load-counts}
  ...
  years <- read.delim("year_counts.tsv", sep="\t")
...
Add a line below using Rmarkdown's inline syntax to print the total number of lines in each dataframe you've created.
   ...
   totals_rows <- nrow(totals)
   years_rows <- nrow(years)
...
years_totals<-
years %>%  
  group_by(year, term) %>% summarize(year_count = sum(volume)) %>% view()
  
proportions<-
  totals %>% 
  full_join(years_totals) %>% mutate(proportion = year_count/total_volume)
...
plot3a %>% 
  ggplot() + geom_line(aes(x = year, y = proportion, color = as.factor(term))) + labs(x ="Year", y = "Frequency", color = "Index") + scale_y_continuous(label=label_percent(scale = 10000,suffix=""))
```
...
plot3a %>% 
  ggplot() + geom_line(aes(x = year, y = year_count, color = as.factor(term))) + labs(x ="Year", y = "Raw Count", color = "Index") + scale_y_continuous(label =comma)
...
```{r plot-totals}
plot3a %>% 
  ggplot() + geom_line(aes(x = year, y = total_volume, color = as.factor(term))) + labs(x ="Year", y = "Totals", color = "Index") + scale_y_continuous(label =comma)
```

## Your written answer

Write up your answer to Part E here.
 Yes, because the graph at (d) suggests that the y value increases over time rather than a downward trend as seen in (b).
...
```{r compute-peaks}
peakmentions <- 
  proportions %>% group_by(term) %>% filter(year >= 1850 & year <= 2012) %>% filter(proportion == max(proportion)) %>% rename(peak_proportion = proportion, peak_year = year)
...
half_lifes <- 
  proportions %>% 
  inner_join(peakmentions, join_by(term)) %>% 
  filter(year >= 1850 & year <= 2012) %>% 
  group_by(term) %>% 
...
```{r plot-half-lifes}
half_lifes %>% 
  ggplot(aes(x=term, y = half_life)) + geom_point() + geom_smooth() + scale_y_log10()
```

## Your written answer

Write up your answer to Part F here.
 No, even though they go down, there is much more fluctuations in the graph I have compared to the book.
...
The years 1800-1809, 1826,1813,1837,1838,1855,1856 seemed like the outliers because they deviated too much from the curve.
  
  half_lifes %>% distinct() %>%  arrange(half_life) %>% head
    
  half_lifes %>%  distinct() %>%  arrange(half_life) %>% tail
