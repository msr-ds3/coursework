...
author: "Your Name (your uni)" Navpreet Kaur
date: '`r Sys.time()`'
output:
  html_document:
    toc: yes
...
ngram_year_counts_and_totals <- merge(year_counts, total_counts, by = "year") %>%
...
head(ngram_year_counts_and_totals)
ngram_year_counts_and_totals %>% 
  filter(term == "1883" | term == "1910" | term == "1950") %>%
  ggplot(aes(x = year, y = proportion_of_mentions, color = factor(term))) + 
  geom_line() + 
...
Answer - The results are very similar to the version 1 results. Some slight differences seem to be that there seems to be more data in the version 2 file, since the values plotted on the graph for the proportion of mentions seem to be slightly higher. For example, in the version 1 graph the 1950 line is slightly under the 0.015% frequency but for the version 2 graph it is slightly above.
...
![The differences between the graphs on the ngram viewer are much more noticeable. Each of the proportions of mentions for each year are much higher which would mean that they have a larger dataset in which these terms show up much more often.](ngrams_viewer.png)
...
Answer - The difference between (b) and (d) and the graph we just plotted lead us to reevaluate some of the results of Michel et al. since we see that in part (d) after plotting the raw counts that the word mentions of a year much after that year has passed can still increase instead of beginning to decrease. For example the line for 1883, continued to increase and even went much higher in the much later years than it was in the years closer to 1883. The graph we just plotted shows us how rapidly the total counts for the mentions of years increase and due to this rapid exponential increase it would be reasonable to reevaluate these results due to this rapid increase of usage for these terms.
...
```{r compute-peaks}
proportion_of_mention_peaks <- ngram_year_counts_and_totals %>%
  filter(year >= 1850 & year <= 2012) %>%
  group_by(term) %>%
  filter(proportion_of_mentions == max(proportion_of_mentions)) %>%
  rename(proportion_of_mentions_highest = proportion_of_mentions) %>%
  rename(peak_year = year)
proportion_of_mention_peaks
...
half_life_df <- inner_join(ngram_year_counts_and_totals, proportion_of_mention_peaks, by = "term") %>%
  filter(year >= 1850 & year <= 2012) %>%
  group_by(term) %>%
  mutate(half_life = abs(peak_year-year)) %>%
  filter(proportion_of_mentions <= 0.5 * proportion_of_mentions_highest) %>%
  #filter(proportion_of_mentions == max(proportion_of_mentions)) %>%
  filter(half_life == min(half_life)) %>%
  rename(decline_year = year)
...
```{r plot-half-lifes}
half_life_df %>% ggplot(aes(x = decline_year, y = half_life)) + 
  geom_point() + 
...
Answer - My version of the plot (not sure if it is correct) is different from the plot produce by the version 1 data. The version 1 data showed that as the years pass by the terms are being used lesser and lesser. The version 2 plot shows a more parabola type relation where at some points the terms being used more and more as the years went on and then decreasing as many more years passed by.
...
Answer - In the graph produced above there did not seem to be any outliers.
```{r}
min(half_life_df$half_life)
max(half_life_df$half_life)
