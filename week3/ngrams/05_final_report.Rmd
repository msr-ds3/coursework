...
```{r load-counts}
#here("ngrams", "year_counts.tsv")
#here("ngrams", "total_counts.csv")

my_yr <- read.table("year_counts.tsv", sep = "\t", header = FALSE, col.names = c("term", "year", "volume", "book_count"))

my_tot <-read.csv("total_counts.csv", col.names = c("year", "total_volume", "page_count", "book_count"))
...
yr_tot <- inner_join(my_yr,my_tot, by=c('year' = 'year')) %>% mutate(prop = 100* (volume / total_volume) )
...
my_graph <- yr_tot %>% filter(term == "1883" | term == "1910" | term == "1950") %>% filter(year > 1849 & year < 2013)

eight_three <- my_graph %>% filter(term == "1883")
nine_ten <- my_graph %>% filter(term == "1910")
nine_five <- my_graph %>% filter(term == "1950")


my_graph %>% ggplot() + geom_line(data = eight_three, aes(x = year, y = prop), color = "blue") + geom_line(data = nine_ten, aes(x = year, y = prop) , color = "green") + geom_line(data = nine_five, aes(x = year, y = prop), color = "red") + scale_y_continuous(labels = scales::percent)
```

## Your written answer

Write up your answer to Part B here.
It seems as years get later, there is a larger spike in references when that year comes, but it is quicker to drop off in subsequent years.
...
It seems that the pattern is relatively similar, but the overall percentages are lower. I assume this is because Google has access to more documents and therefore the percentage is smaller
...
my_graph %>% ggplot() + geom_line(data = eight_three, aes(x = year, y = volume), color = "blue") + geom_line(data = nine_ten, aes(x = year, y = volume) , color = "green") + geom_line(data = nine_five, aes(x = year, y = volume), color = "red") + scale_y_continuous(labels = scales::comma)
```
...
```{r plot-totals}
my_graph %>% ggplot() + geom_line(data = yr_tot, aes(x = year, y = total_volume), color = "blue") + scale_y_continuous(labels = scales::comma)
```
...
Yeah it absolutely does. This suggests that the proportion of mentions about a year going down isn't due to less documents being made about it as time goes on, it's due to there being way more documents made total.
...
```{r compute-peaks}
my_graph %>% group_by(term) %>% filter(prop == max(prop))
```
...
yr_tot <-yr_tot %>% filter(year > 1849 & year < 2013) %>% filter(term > 1849 & term < 2013)

max_yrs <- yr_tot %>% group_by(term) %>% filter(prop == max(prop)) %>% select(term,big_year = year, big_prop = prop) %>% arrange(term)


tetsy <- yr_tot %>% inner_join(max_yrs, "term") %>% group_by(term) %>% filter(prop >= (big_prop/2) & year > big_year) %>% group_by(term) %>% filter(year == max(year)) %>% mutate(half = year - big_year) %>% arrange(term) %>% mutate(year = as.numeric(year))
...
```{r plot-half-lifes}
ggplot(tetsy, aes(x = year, y = half)) + geom_point() + scale_y_log10() + geom_smooth()
```

## Your written answer

Write up your answer to Part F here.
It is very different. There are much smaller differences for every half life. 
...
1860 took 80 years to reach its half life. Everything else is super close to 0 so it's unlikely that any of them can be considered outliers. I assume it's the Civil War that was refenced very often. As it took place in teh 1860's.

```{r}
tetsy %>% arrange(half)
tetsy %>% arrange(desc(half))
```
