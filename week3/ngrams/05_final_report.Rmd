...
year_total_counts <- year_counts |> 
  inner_join(total_counts, join_by(year))
  
proportion_table <- year_total_counts |> 
  group_by(term, year) |> 
  summarise(term_per_year = sum(volume),
            proportion = term_per_year/total_volume)
```
...
proportion_table |> 
  ...
  scale_y_continuous(labels = label_percent(scale = 10000, suffix = "")) +
...
We found that the years before 1883/1910/1950 had larger spikes when the years were mentioned before they occurred
...
```{r}
knitr::include_graphics('Screenshot 2024-06-20 133801.png')
```

We found that the graphs were similar from the year 1800 to the present, but our unfiltered graph contained peaks in the year before 1800.
# Part D
...
unproportion_table <- year_total_counts |> 
  group_by(term, year) |> 
  summarise(term_per_year = sum(volume))

unproportion_table |> 
  filter(term == c("1883", "1910", "1950")) |> 
  ggplot(aes(x = year, y = term_per_year)) +
  geom_line(aes(color = as.factor(term))) +
...
Because there are so many more words that are being published per year as time goes on, the total volume increases and the number of times the terms were mentioned per year is overshadowed by the magnitude of words that are being produced that year.
So while the actual number of times the terms are used is increasing, the proportion is decreasing.
...
The results are not that similar. The dips and peaks don't match those in the paper. It does have the same general trend that the half-lifes are decreasing.
...
The years with the longest half-lives are both years that would be more eventful and they all take place towards the beginning of the time frame. We think that although there are years that were more eventful, they suffer from proportion bias and therefore due not have long half-lives while the earlier years do not suffer from this. The years that have short half-lives were uneventful.
