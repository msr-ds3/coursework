...
```{r load-counts}
year_counts.tsv <- here("year_counts.tsv")
total_counts.csv <- here("total_counts.csv")

yc <- read.table(file = "year_counts.tsv", sep = '\t', header = FALSE, col.names = c("term", "year", "volume", "book_count"))

tc <- read.table(file = "total_counts.csv", sep = ',', header = FALSE, col.names = c("year", "total_volume", "page_count", "book_count"))
```
...
yc_tc <- inner_join(yc, tc, "year") %>% mutate(prop = (volume/total_volume)*100)
...
smaller_yc_tc <- yc_tc %>% filter(year > 1849 &  year < 2013)
ggplot() + 
  geom_line(filter(smaller_yc_tc, term == "1883"), mapping = aes(x = year, y = prop), color = "blue") +
  geom_line(filter(smaller_yc_tc, term == "1910"), mapping = aes(x = year, y = prop), color = "green") +
  geom_line(filter(smaller_yc_tc, term == "1950"),mapping = aes(x = year, y = prop), color = "red") + 
  xlab("Year") + scale_y_continuous(labels = scales::percent) 
```

## Your written answer

Write up your answer to Part B here.
It seems that there is a peak of mentions for each selected year a couple of years after the year selected and then the mentions quickly go down.  
...
## Your written answer
The graphs are very similar , however the percentages on the y-axis are lower in Google's chart. I assume this happens because Google has access to more books and so the percentage of appearences for these specific terms are much smaller. 
Additionally, Google's graph is much smoother and doesn't show all the "bumps" that each line has in mine.
...
ggplot() + 
  geom_line(filter(smaller_yc_tc, term == "1883"), mapping = aes(x = year, y = volume), color = "blue") +
  geom_line(filter(smaller_yc_tc, term == "1910"), mapping = aes(x = year, y = volume), color = "green") +
  geom_line(filter(smaller_yc_tc, term == "1950"),mapping = aes(x = year, y = volume), color = "red") + 
  xlab("Year") + scale_y_continuous(labels = scales::comma) 
```
...
```{r plot-totals}
ggplot() + 
  geom_line(smaller_yc_tc, mapping = aes(x = year, y = total_volume)) +
  xlab("Year") + ylab("Frequency") + scale_y_continuous(labels = scales::comma) 
```
...
There is a big difference that led me to reevaluate the results of Michel et. al. (2001). Considering the total_volume of terms per year, and not the percentages of each term compared to the total, made me understand that we did not start forgetting faster and faster (stopped making documents about older terms), as the author of the article claims. Instead, the number of digitalized books (total number of terms) after the year 1950 significantly increased, and that is the only reason why the percentages for older terms become lower with time. If we look at the total appearances, we actually talk about older terms in recent years more than in the past. In other words, it is not the case that people are forgetting faster and faster, instead, the d graph shows that the percentage of appearances of "older" terms gets smaller and smaller only compared to the total number of terms, because the number of digitalized documents increases exponentially year by year (also due to technology).
...
```{r compute-peaks}
peak_yc_tc <- smaller_yc_tc %>% group_by(term) %>% filter(prop == max(prop)) %>% select(term, "year_peak"= year)
```
...
half_times <- inner_join(smaller_yc_tc, peak_yc_tc, "term") %>%  group_by(term) %>% filter(prop >= (max(prop)/2), year > year_peak) %>%  group_by(term) %>% filter(year == max(year)) %>% mutate(half_time = (year - year_peak))
...
half_times %>% ggplot() + 
  geom_point(mapping = aes(x = year, y = half_time)) + geom_smooth(mapping = aes(x = year, y = half_time)) +
  xlab("Year") 
```
...
## Your written answer
The years that were forgotten particularly quickly are 1857, 1856 because there are fewer digitalized documents that include them compared to more recent years. At the same time, 1800 is the year with the largest half life because it was a very important year in history so it is mentioned in many historical books.

Write up your answer to Part G here. Include code that shows the years with the smallest and largest half-lifes.
```{r}
largest_half_time <- max(half_times$half_time)
lg_year <- filter(half_times, half_time == largest_half_time)$term
smallest_half_time <- min(half_times$half_time)
sm_year <- filter(half_times, half_time == smallest_half_time)$term
```
