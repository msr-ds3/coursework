...
year_counts <- read_tsv(here("C:/Users/yino/coursework/week3/ngrams/year_counts.tsv"), col_names = c("term", "year", "volume", "book_count"))
total_counts <- read_csv(here("C:/Users/yino/coursework/week3/ngrams/total_counts.csv"), col_names = c("year", "total_volume", "page_count", "book_count"), col_types = cols(
  year = col_integer(),
...
total_counts$year <- as.integer(total_counts$year)

# Check for common years between the two datasets
common_years <- intersect(year_counts$year, total_counts$year)
length(common_years)

# Perform the inner join
data <- year_counts %>%
  inner_join(total_counts, by = "year") %>%
...
data_filtered <- data %>% 
  filter(term %in% c("1883", "1910", "1950") & year >= 1875)

ggplot(data_filtered, aes(x = year, y = proportion, color = factor(term))) +
  geom_line() +
  scale_y_continuous(labels = percent) +
  scale_color_manual(values = c("1883" = "darkblue", "1910" = "darkgreen", "1950" = "red"))+
  labs(title = "Usage Trajectories for Specific Years",
       x = "Year",
       y = "Frequency")+ scale_x_continuous(breaks=seq(1875,2000,by=25))
...
ggplot(data_filtered, aes(x = year, y = volume, color = factor(term))) +
  geom_line() +
  scale_y_continuous(labels = comma) +
  labs(title = "Raw Mentions for Specific Years",
       x = "Year",
       y = "Raw Mentions")
```
...
ggplot(total_counts, aes(x = year, y = total_volume)) +
  geom_line() +
  scale_y_continuous(labels = comma) +
  labs(title = "Total Counts Over Time",
       x = "Year",
       y = "Total Volume")
...
peak_mentions <- data %>%
  group_by(term) %>%
  summarize(peak_year = year[which.max(proportion)],
            peak_proportion = max(proportion, na.rm = TRUE))
...
half_lifes <- data %>%
  inner_join(peak_mentions, by = "term") %>%
  group_by(term) %>%
  summarize(
    peak_year = unique(peak_year),
    peak_proportion = unique(peak_proportion),
    half_life = {
      half_year <- year[proportion <= (peak_proportion / 2) & year > peak_year]
      if (length(half_year) > 0) {
        min(half_year) - peak_year
      } else {
        NA  
      }
    }
  ) %>%
  filter(!is.na(half_life), peak_year >= 1875 & peak_year <= 1975)
...
clean_half_lifes <- half_lifes %>%
  filter(is.finite(half_life))

highlight_years <- c(1883, 1910, 1950)
highlight_data <- clean_half_lifes %>% filter(peak_year %in% highlight_years)

ggplot(clean_half_lifes, aes(x = peak_year, y = half_life)) +
  geom_point(alpha = 0.5, color = "grey", size = 2) + 
  geom_point(data = highlight_data, aes(x = peak_year, y = half_life), 
             color = c("blue", "green", "red"), size = 4) + 
  geom_smooth(method = "loess", span = 0.5, se = FALSE, color = "black", size = 1) + 
  labs(title = "Half-life Over Time",
       x = "Year",
       y = "Half-life (yrs)") +
  theme_minimal() +
  ylim(0, 40) +  
...
```{r}
#  outliers
quickly_forgotten <- clean_half_lifes %>% arrange(half_life) %>% head(5)
slowly_forgotten <- clean_half_lifes %>% arrange(desc(half_life)) %>% head(5)

quickly_forgotten
slowly_forgotten

```
Quickly Forgotten Years might correspond to minor events. For example, years during periods of relative stability or minor political changes might be forgotten more rapidly.
Slowly Forgotten Years: These years could correspond to major historical events that had long-lasting impacts. For example, years with significant wars or revolutions tend to have a longer-lasting impact.
