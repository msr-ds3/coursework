...
trips_test <- left_join(trips_test, holidays)
trips_test[is.na(trips_test)] = 0

#include weekdays and factorize them
trips_test$weekday <- wday(trips_test$ymd, week_start=1)
trips_test$weekday <- as.factor(trips_test$weekday)

trips_2015 <- inner_join(trips_test, weather)

# save data frame for easy loading in the future
save(trips_2015, weather, file='trips_2015.RData')
