...
## 1. Exploring Raw Correlations

**Next, we’ll peek at the correlations** to see how these variables vary together. (Remember, seeing a correlation doesn’t mean one thing /causes/ the other.)

```{r cor-matrix}
df %>%
  select(Age, Height, ShoeSize, ReadingScore) %>%
  cor() %>%
  round(2)
```

**Then** we’ll **plot shoe size versus reading score** to visualize onne of these associations:
...
```{r tidy-summaries}
bind_rows(
  tidy(lm(ReadingScore ~ ShoeSize, df))       %>% mutate(model = '1. ShoeSize only'),
