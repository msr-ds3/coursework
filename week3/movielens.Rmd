...
# a dataframe with the 100% satified and 90% satisfied rank for each user
user_sat <-
  ratings_with_rank %>%
  group_by( user_id ) %>%
  summarize( hundred = max( rank ), ninety = rank[floor(n() * 0.9)] )

plot100 <- user_sat%>%
  group_by(hundred)%>%
  summarize(user_satisfied = n())%>%
  arrange(desc(user_satisfied))%>%
  mutate(inventory = row_number(), frac_user_satisfied = cumsum(user_satisfied)/sum(user_satisfied))
...
ggplot() +
  geom_line( data = plot100, aes(x = inventory, y = frac_user_satisfied) ) +
  geom_line( data = plot90, aes(x = inventory, y = frac_user_satisfied) ) +
