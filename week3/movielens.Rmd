...
ratings |> 
  ...
  ggplot(aes(x = mean_ratings)) +
  geom_histogram()
ratings |> 
  group_by(movie_id) |> 
  summarise(mean_ratings = mean(rating)) |> 
  ggplot(aes(x = mean_ratings)) +
  geom_density(fill = "gray", alpha = .3)
...
ratings_user_movie_pop <- ratings_by_user_movie |> 
  left_join(movie_popularity) |> 
  select(user_id, movie_id, rank)

hundred <- ratings_user_movie_pop |> 
  group_by(user_id) |> 
  mutate(least_pop = max(rank)) |> 
  summarise(satisfied = mean(least_pop)) |> 
  arrange(satisfied) |>
  group_by(satisfied) |> 
  summarise(count_satisfied = n()) |> 
  mutate(frac_satisfied = cumsum(count_satisfied)/sum(count_satisfied))


ninety <- ratings_user_movie_pop |> 
  group_by(user_id) |> 
  arrange(rank, .by_group = TRUE) |>
  mutate(ninety_satisfied = rank[as.integer(n()*.9)]) |>
  summarise(satisfied = mean(ninety_satisfied)) |> 
  arrange(satisfied) |>
  group_by(satisfied) |> 
  summarise(count_satisfied = n()) |> 
  mutate(frac_satisfied = cumsum(count_satisfied)/sum(count_satisfied)) 

ggplot(hundred, aes(x = satisfied, y = frac_satisfied)) +
  geom_line() + 
  geom_line(data = ninety) +
