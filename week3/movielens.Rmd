---
title: "Movielens"
date: '`r Sys.time()`'
output:
  html_document:
    #code_folding: hide
    number_sections: yes
    toc: yes
    toc_depth: 3
---

```{r setup, include=FALSE}
library(scales)
library(tidyverse)
library(knitr)

# set plot theme
theme_set(theme_bw())
```

# Load and preview data

Read data from the `ratings.csv` file
```{r load-data}
ratings <- read_csv('ratings.csv',
                    col_names = c('user_id','movie_id','rating','timestamp'))
```

Loaded `r format(object.size(ratings), units="Mb")` of ratings data, containing `r format(nrow(ratings), big.mark = ",")` ratings. Here's a preview:
```{r preview-data}
head(ratings) %>% kable()
```

# Summary statistics

```{r dist-ratings}
# plot the distribution of rating values https://speakerdeck.com/jhofman/modeling-social-data-lecture-2-introduction-to-counting?slide=26

ratings %>% 
  group_by(rating) %>%
  ggplot(aes(x=rating)) +
  geom_bar(fill='red') +
  scale_y_continuous(labels=comma) + 
  labs(
    x='Rating',
    y='Number of Ratings',
    title='How many ratings are at each star level?'
  )


```

## Per-movie stats

```{r aggregate-by-movie}
# aggregate ratings by movie, computing mean rating and number of ratings
# hint: use the n() function for easy counting within a group

movie_stats <- ratings %>%
  group_by(movie_id) %>%
  summarize(
    count=n(),
    mean=mean(rating)
  )

movie_stats

```

```{r dist-movie-popularity}
# plot distribution of movie popularity (= number of ratings the movie received)
# hint: try scale_x_log10() for a logarithmic x axis

movie_stats %>%
  ggplot(aes(x=count)) +
  scale_x_log10(label=comma) +
  geom_histogram(color='darkblue', fill='lightblue') + 
  labs(
    x='Number of Ratings',
    y='Number of Movies',
    title='Distribution of Movie Popularity'
  )

```

```{r dist-mean-ratings-by-movie}
# plot distribution of mean ratings by movie https://speakerdeck.com/jhofman/modeling-social-data-lecture-2-introduction-to-counting?slide=28
# hint: try geom_histogram and geom_density

movie_stats %>%
  ggplot(aes(x=mean)) +
  geom_histogram() +
  labs(
    x='Mean Rating',
    y='Amount for Rating',
    title='Distribution of Mean Ratings by Movie'
  )

movie_stats %>%
  ggplot(aes(x=mean)) +
  geom_density(fill='black') +
  labs(
    x='Mean Rating',
    y='Amount for Rating',
    title='Distribution of Mean Ratings by Movie'
  )

```

```{r cdf-movie-pop}
# rank movies by popularity (number of ratings) and compute the cdf, or fraction of all views covered by the top-k movies https://speakerdeck.com/jhofman/modeling-social-data-lecture-2-introduction-to-counting?slide=30
# hint: use dplyr's rank and arrange functions, and the base R sum and cumsum functions
# store the result in a new data frame so you can use it in creating figure 2 from the paper below

# plot the CDF of movie popularity
library(dplyr)

movie_stats %>%
  arrange(desc(count)) %>%
  mutate(
    rank=row_number(),
    cdf=(cumsum(count) / sum(count)) * 100
  ) %>%
  ggplot(aes(x=rank, y=cdf)) +
  geom_line() +
  labs(
    x='Movie Rank',
    y='CDF',
    title='Cumulative Density of Movies by Popularity'
  )


```


# Per-user stats

```{r aggregate-by-user}
# aggregate ratings by user, computing mean and number of ratings

user_stats <- ratings %>%
  group_by(user_id) %>%
  summarize(
    count=n(),
    mean=mean(rating)
  )

user_stats

```

```{r dist-user-activity}
# plot distribution of user activity (= number of ratings the user made)
# hint: try a log scale here

user_stats %>%
  ggplot(aes(x=count)) +
  scale_x_log10(label=comma) +
  geom_histogram(bins=30, color='darkblue', fill='lightblue') +
  labs(
    x='Number of Ratings',
    y='Number of Users',
    title='Distribution of User Activity'
  )

```

# Anatomy of the long tail

```{r long-tail}
# generate the equivalent of figure 2a of this paper:
# note: don't worry about the "null model" lines
# just do the solid lines and dotted line (optional)
# https://5harad.com/papers/long_tail.pdf

# Specifically, for the subset of users who rated at least 10 movies,
# produce a plot that shows the fraction of users satisfied (vertical
# axis) as a function of inventory size (horizontal axis). We will
# define "satisfied" as follows: an individual user is satisfied p% of
# the time at inventory of size k if at least p% of the movies they
# rated are contained in the top k most popular movies. As in the
# paper, produce one curve for the 100% user satisfaction level and
# another for 90%---do not, however, bother implementing the null
# model (shown in the dashed lines).

users_above_ten <- ratings %>%
  group_by(user_id) %>%
  summarize(user_count = n()) %>%
  filter(user_count >= 10) %>%
  pull(user_id)

num_users <- length(users_above_ten)

ratings_with_users_above_ten <- ratings %>% 
  filter(user_id %in% users_above_ten)
  
ratings_aug <- ratings %>% 
  group_by(movie_id) %>% 
  summarize(movie_count=n()) %>% 
  arrange(desc(movie_count)) %>% 
  mutate(rank = row_number()) %>%
  inner_join(ratings_with_users_above_ten)
  
curve100 <- ratings_aug %>%
  group_by(user_id) %>% 
  summarize(
    percentile=quantile(rank, 1.0),
  ) %>% 
  arrange(percentile) %>%
  mutate(fraction_100_satisfied=(row_number()/num_users)*100) %>%
  pivot_longer(
    cols=starts_with('fraction'),
    names_to='fraction',
    values_to='stat'
  ) 

curve90 <- ratings_aug %>% 
  group_by(user_id) %>% 
  summarize(
    percentile=quantile(rank, 0.9)
  ) %>% 
  arrange(percentile) %>% 
  mutate(fraction_90_satisfied=(row_number()/num_users)*100) %>% 
  pivot_longer(
    cols=starts_with('fraction'),
    names_to='fraction',
    values_to='stat'
  )  

bind_rows(curve100, curve90) %>%
  arrange(percentile) %>% 
  ggplot(aes(x=percentile, y=stat, color=fraction)) +
  geom_line() + 
  labs(
    x = 'Inventory Size',
    y = '% Users Satisfied',
    title = 'User Satisfaction by Inventory Size',
    color= '% Satisfaction'
  ) +
  scale_color_discrete(
    labels=c('100% Satisfied', '90% Satisfied')
  ) + 
  geom_vline(xintercept=3000, linetype="dashed")

```
