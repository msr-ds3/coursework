...
### save trips_2015
save(trips_2020, file="justTrips_2020.RData")
...
trips_test_2020 <- trips_2020%>% group_by(ymd) %>% summarize(num_trips_2020= n())
trips_test2_2020 <-  trips_2020%>% group_by(ymd) %>% summarize(num_trips_2020= n())
...
trips_test_2020 <- right_join(holidays, trips_test_2020, "ymd") %>% 
...
# join weather and trips_2020
trips_test_2020 <- inner_join(weather, trips_test_2020, "ymd") %>% 
  mutate(weather = ifelse(prcp > 0, "Raining", "Good_weather")) %>% 
  mutate(weather = ifelse(snwd > 0, "Snowing", weather))

View(trips_test_2020)
# save data frame for easy loading in the future
save(trips_test_2020, weather, file='trips_2020.RData')
