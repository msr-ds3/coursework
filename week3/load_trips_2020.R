...
# before doing the step below, create a new folder in week3 and move all the data from 2015 to the folder.
...
trips_per_day_2020 <- trips %>% select(ymd)

trips_per_day_2020 <- trips_per_day_2020 %>% group_by(ymd) %>% summarize(num_trips = n())

weather_2020 <- weather %>% filter(year(ymd) == 2020)

weather_2020 <- weather_2020 %>% select(prcp, snwd, snow, tmax, tmin, ymd)

trips_per_day_2020 <- merge(trips_per_day_2020, weather_2020, by = "ymd")
