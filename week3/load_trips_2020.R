...
csvs <- Sys.glob('2020*-tripdata.csv')
trips <- data.frame()
for (csv in csvs) {
  print(csv)
...
trips_test_2020 <-trips %>% group_by(ymd) %>% summarize(num_trips = n())

holidays <- read.table('holidays', sep = ',', header = FALSE, col.names = c("row", "ymd", "holiday_name")) %>%
  select(ymd) %>%
  mutate(ymd = as.Date(ymd), is_holiday = 1)

trips_test_2020 <- left_join(trips_test_2020, holidays)
trips_test_2020[is.na(trips_test_2020)] = 0

#include weekdays and factorize them
trips_test_2020$weekday <- wday(trips_test_2020$ymd, week_start=1)
trips_test_2020$weekday <- as.factor(trips_test_2020$weekday)

trips_test_2020 <- inner_join(trips_test_2020, weather)

trips_2020 <- trips_test_2020

# save data frame for easy loading in the future
save(trips_2020, weather, file='trips_2020.RData')
