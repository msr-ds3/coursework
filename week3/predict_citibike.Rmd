...
trips %>%
  mutate(holiday = isHoliday(ymd, "USFed")) %>%
  mutate(wday = wday(ymd), month = month(ymd, label = T)) %>%
  ggplot(aes(x = tmax, y = num_trips, color = factor(month))) +
  geom_point()
...
for (n in N) {
  ...
  for (f in 1:num_folds) {
    ...
    model <- lm(num_trips ~ poly(tmin, n, raw = T) * poly(tmax, n), data = train_data)
...
for (f in 1:num_folds) {
  ...
  # fit on training data
  train_data <- filter(train, fold != f)
  model <- lm(num_trips ~  isHoliday(dates = ymd, type = "USFed") + factor(wday(ymd)) + factor(months(ymd)) + snwd + I(snow > 0) + prcp + poly(tmin, 2, raw = T) * poly(tmax, 2, raw = T), data = train_data)
...
validation_data %>%
  ...
  geom_point() +
  geom_abline() + 
  labs(x = "predictions", y = "No. of trips", title = "Actual vs Predicted Plot")
