...
```{r}
bank_holidays <- read_csv(here('week3/bank_holiday.csv'), col_names=c("holidy_id", "ymd", "holiday_name"))
...
```
Okay. I originally got a number in the 2000s, and I was surprised by how low it was. But when Fabio ran it, he got this. It looks like this number (3162.115) is the number you get with the actual seed (I had rerun it with a different seed to check that the result didn't change too much - it didn't - and then forgot to get the seed back). But this is almost identical to the 3113 result we got on out cross-validation. So, it looks like we aren't overfitting.

But, if we get a future year, I suspect we will do worse, because Citibike itself could have had changing popularity. If their popularity went up, then we will consistently underestimate, and vice versa.
