...
model5 <- lm(num_trips ~ poly(tmin, 5, raw=T), data=trips_per_day_train)
...
# test the model
test_err <- sqrt(mean((predict(model5, trips_per_day_test) - trips_per_day_test$num_trips)^2))
test_err
...
for (k in K) {
  ...
  # fit on the training data
  model <- lm(num_trips ~ poly(tmin, k, raw=T) + too_rainy, data=trips_per_day_with_rain_train)
  
  # evaluate on the training data
  train_err[k] <- sqrt(mean((predict(model, trips_per_day_with_rain_train) - trips_per_day_with_rain_train$num_trips)^2))
  
  # evaluate on the validate data
  validate_err[k] <- sqrt(mean((predict(model, trips_per_day_with_rain_validate) - trips_per_day_with_rain_validate$num_trips)^2))
}
...
for (k in K) {
  ...
  # fit on the training data
  model <- lm(num_trips ~ poly(tmin, k, raw=T) + is_weekend, data=trips_per_day_with_weekends_train)
  
  # evaluate on the training data
  train_err[k] <- sqrt(mean((predict(model, trips_per_day_with_weekends_train) - trips_per_day_with_weekends_train$num_trips)^2))
  
  # evaluate on the validate data
  validate_err[k] <- sqrt(mean((predict(model, trips_per_day_with_weekends_validate) - trips_per_day_with_weekends_validate$num_trips)^2))
}
...
for (k in K) {
  ...
  # fit on the training data
  model <- lm(num_trips ~ poly(tmin, k, raw=T) + too_rainy + is_weekend + poly(tmax, k, raw=T), data=trips_per_day_with_all_info_train)
  
  # evaluate on the training data
  train_err[k] <- sqrt(mean((predict(model, trips_per_day_with_all_info_train) - trips_per_day_with_all_info_train$num_trips)^2))
  
  # evaluate on the validate data
  validate_err[k] <- sqrt(mean((predict(model, trips_per_day_with_all_info_validate) - trips_per_day_with_all_info_validate$num_trips)^2))
}
...
model3 <- lm(num_trips ~ poly(tmin, 3, raw=T) + too_rainy + is_weekend + tmax, data=trips_per_day_with_all_info_train)
...
rsquare(model3, trips_per_day_with_all_info_train)
rsquare(model3, trips_per_day_with_all_info_validate)
...
# model for degree 3 polynomial
model4 <- lm(num_trips ~ poly(tmin, 4, raw=T) + too_rainy + is_weekend + tmax, data=trips_per_day_with_all_info_train)
...
rsquare(model4, trips_per_day_with_all_info_train)
rsquare(model4, trips_per_day_with_all_info_validate)
