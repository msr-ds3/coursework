...
```{r}
model1 <- lm(num_trips ~ poly(tmin, 5, raw = T), data = train_set)
...
```
The RMSE of 4690.563 is the lowest I've gotten after attempting multiple different arrangements of different variables.
...
model_final <- lm(num_trips ~ poly(tmin, 4, raw = T) + sub_rain + prcp + snow + snwd + weekday + is_holiday + tmax, data = trips_validate)
