...
#join trips+weather
joined <- inner_join(weather, trips, by = "ymd")

trips_per_day_2015 <- joined %>% 
  mutate(tmin = tmin/10, tmax = tmax/10, dow = wday(ymd), is_weekend = as.numeric((dow > 5))) %>% 
  add_count(ymd, name = 'num_trips') %>% 
  select(ymd, num_trips, prcp, snwd, snow, tmax, tmin, dow, is_weekend) %>% 
  unique()

save(trips_per_day_2015, file = "trips_per_day_2015.Rdata")
