...
```{r}
# model <- lm(num_trips ~ poly(tmin, 3, raw = T) + prcp, data=trips_per_day_train)
...
```
```{r}
trips_per_day_train_dow <- trips_per_day_train %>%
  mutate(dow = wday(ymd))

trips_per_day_validate_dow <- trips_per_day_validate %>% 
  mutate(dow = wday(ymd))

days_of_week <- data.frame(dow = rep(1:7), is_weekend = c(0,0,0,0,0,1,1))

trips_per_day_train_dow <- left_join(trips_per_day_train_dow, days_of_week, by = "dow")

trips_per_day_validate_dow <- left_join(trips_per_day_validate_dow, days_of_week, by = "dow")

```

```{r}
model <- lm(num_trips ~ poly(tmin, 4, raw = T) + is_weekend + prcp + snwd, trips_per_day_train_dow)

trips_train <- trips_per_day_train_dow %>%
  add_predictions(model) %>%
  mutate(split = "train")
trips_validate <- trips_per_day_validate_dow %>%
  add_predictions(model) %>%
  mutate(split = "validate")
