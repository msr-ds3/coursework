...
trips_test <- trips_test %>%
  mutate(day_name = weekdays(ymd),
         snow_high = ifelse(snwd > mean(snwd), 1, 0),
         avg_temp = (tmin + tmax) / 2,
         prcp_high = ifelse(prcp > 0.15, 1, 0),
         month = month(ymd),
         season = case_when(
           month %in% c(12, 1, 2) ~ "Winter",
           month %in% c(3, 4, 5) ~ "Spring",
         ...
         day_type = ifelse(day_name %in% c("Saturday", "Sunday"), 1, 0)) %>%
  select(-day_name)
head(trips_test)
test_data_w_prediction <- mutate(trips_test, predicted_num_trips = predict(model, trips_test)) 
...
ggplot(test_data_w_prediction, aes(x = ymd, y = num_trips)) +
  ...
  theme_minimal()
```
This plot shows the predicted number of trips (in red) and the actual number of trips (in blue) on the test data. The model seems to perform well on the test data as well, capturing the overall trend in the number of trips with RMSE of 3358.9