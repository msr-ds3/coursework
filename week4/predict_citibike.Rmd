...
My exploration is complete! Below is my final model, and it is looking great! My RMSE is now just above 3100 and I have super high R-squared and adjusted R-squared values of around 93%!

```{r final-model}
model <- lm(num_trips ~ I(prcp > avg_prcp)*tmin + I(prcp > hlfsdup_prcp)*tmax + 
              snwd*tmin + snow*tmin + snow*snwd + holiday + weektype + 
              poly(month, 4, raw=T) + poly(tmax, 4, raw=T) + poly(tmin, 4, raw=T),
            data=trips_per_day_model)
summary(model)
```
...
```{r plot-data}
trips_train <- trips_per_day_train %>%
  add_predictions(model) %>%
  mutate(split = "train")
trips_validate <- trips_per_day_validate %>%
  add_predictions(model) %>%
