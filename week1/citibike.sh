...
# find the day with the most rides
 cut -d, -f2 201402-citibike-tripdata.csv | cut -d" " -f1 | tr -d '"' | sort | uniq -c | sort | tail -n1  
 
 cut -d, -f2 201402-citibike-tripdata.csv | cut -c 2-11 | sort | uniq -c | sort | tail -n1

# find the day with the fewest rides
 cut -d, -f2 201402-citibike-tripdata.csv | cut -d" " -f1 | tr -d '"' | sort | uniq -c | sort -r| tail -n2  
 cut -d, -f2 201402-citibike-tripdata.csv | cut -c 2-11 | sort | uniq -c | sort -r| tail -n2
# find the id of the bike with the most rides
 cut -d, -f12 201402-citibike-tripd
ata.csv | sort | uniq -c | sort | tail -n1
# count the number of rides by gender and birth year
 cut -d, -f14,15 201402-citib
ike-tripdata.csv |sort | uniq -c
