...
filter(trips, grepl(".[0-9]", birth_year)) %>% 
  summarize(min(birth_year), max(birth_year))
...
group_by(trips, gender) %>%
  summarize(avg_trip_time=mean(tripduration), sd_trip_time=sd(tripduration))
...
mutate(trips, hour = hour(starttime)) %>%
  group_by(hour) %>%
  count() %>%
  mutate(avg_trips_per_hour = n/28) %>%
  arrange(desc(avg_trips_per_hour))
