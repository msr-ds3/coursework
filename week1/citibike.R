...
filter(trips, grepl('Broadway',trips$start_station_name) | grepl('Broadway', trips$end_station_name)) %>% view
...
summarise(trips_by_gender, mean_trips_time_by_gender=mean(tripduration))
summarise(trips_by_gender, sd_trips_time_by_gender=sd(tripduration))

# do this all at once, by using summarize() with multiple arguments

summarize(trips_by_gender, 
          count =n(), 
          mean_trips_time_by_gender = mean(tripduration),
...
# find the top 3 most common station-to-station trips by gender
 trips %>% select(gender, start_station_name, end_station_name) %>% 
