...
#head(summarize(group_by(trips, start_station_name, end_station_name), count = n(), desc(count)), 10)
 trips %>% 
...
# head(summarize(group_by(trips, end_station_name, start_station_name), count = n(), desc(count)), 3)
trips %>% 
...
#head(arrange(summarize(group_by(trips, gender, end_station_name, start_station_name), count = n(), desc(count))), 3)
#trips %>% 
#  group_by(gender) %>%
 # summarize(count = n()) 
...
trips <- mutate(trips, date = as.Date(starttime))
trips %>%
  group_by(date) %>%
  summarize(trips = n()) %>%
  arrange(desc(trips)) %>%
