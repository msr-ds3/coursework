...
trips |> 
  ...
  summarise(max_value = max(birth_year, na.rm = T), 
            min_value = min(birth_year, na.rm = T))
...
trips |> 
  mutate(tripduration_in_minutes = tripduration / 60) |> 
  group_by(gender) |> 
  summarise(count = n(),
            average = mean(tripduration_in_minutes),
            standard_deviation = sd(tripduration_in_minutes))
...
trips |> 
  ...
  summarize(num_trips = n(),
            ...
            avg_num_trips = num_trips / num_days)
