...
trips <- read_csv('C:/Users/jane/Documents/Github/coursework/week1/201402-citibike-tripdata.csv')
...
trips %>%
  group_by(gender) %>%
  summarize(sd_by_gender = sd(tripduration)) %>%
  View()
...
trips%>%
  ...
  group_by(hour)%>%
  summarize(avg_trip = n()/n_distinct(as.Date(starttime)))%>%View()
...
trips%>%
  mutate(hour = hour(starttime))%>%
  group_by(hour)%>%
  summarize(avg_trip = n()/n_distinct(as.Date(starttime)))%>%
  arrange(desc(avg_trip))%>%View()