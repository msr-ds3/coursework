...
trips %>%ggplot(aes(x=tripduration)) + geom_histogram()+scale_x_log10(label = comma )
...
trips %>%
mutate(day = floor_date(starttime,unit="day"))%>%group_by(day)%>%summarize(count = n())%>% ggplot(aes(x = day,y = count)) + geom_line()
...
weather%>%mutate(day = floor_date(ymd,unit="day"))%>%ggplot(aes(x = day , y = tmin))+ geom_point()  +scale_y_log10(label = comma )
...
trips_with_weather%>%group_by(ymd)%>%summarize(count=n(), mean = mean(tmin))%>% ggplot(aes(x=mean, y = count))
 +geom_point()
# repeat this, splitting results by whether there was substantial precipitation or not
# you'll need to decide what constitutes "substantial precipitation" and create a new T/F column to indicate this
trips_with_weather%>% mutate(prcptf = ifelse (prcp > mean(prcp) , "T", "F")) %>% group_by(ymd,prcptf)%>%
summarize(count=n(), mean = mean(tmin))%>% 
ggplot(aes(x=mean, y = count)) +geom_point()
#+facet_wrap(~prcptf)

# add a smoothed fit on top of the previous plot, using geom_smooth
trips_with_weather%>% mutate(prcptf = ifelse (prcp > mean(prcp) , "T", "F")) %>% group_by(ymd,prcptf)%>%
summarize(count=n(), mean = mean(tmin))%>% 
ggplot(aes(x=mean, y = count)) +geom_point()+geom_smooth(method = "lm")
